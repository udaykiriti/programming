...
LIBC_INLINE double lgamma_positive_d(double x) {
  ...
  if(x >= 3.0f){
    log_product = math::log(static_cast<double>(x));
    xs = x + 1.0;
  } else if(x >= 2.0f){
    log_product = math::log(static_cast<double>(x) * (x + 1.0));
    xs = x + 2.0;
  } else{
    log_product = math::log(static_cast<double>(x) * (x + 1.0) * (x + 2.0));
    xs = x + 3.0;
  }
...
LIBC_INLINE bfloat16 lgammabf16(bfloat16 x) {
  ...
  if (LIBC_UNLIKELY(x_bits.is_neg())) {
    ...
    constexpr double DC[4] = {-0x1.5555555555555p-3 /* -1/6 */,
    ...
    */
    double sin_pi_frac_d =
        ...
        (1.0 +
         x_pi2_d *
             (DC[1] + x_pi2_d * (DC[2] + x_pi2_d * (DC[3] + x_pi2_d * DC[4]))));
